...
//					tk.now.allocation();
	...
	this.setQwertyLayout = function(id, div, isMultiCursor, _cssName, keyboardType){ // bwlim
		qwertyLayout_noinline(id, div, isMultiCursor, _cssName, keyboardType);
	};
...
//		    document.getElementsByTagName("head")[0].appendChild(link);
	...
	function qwertyLayout_noinline(id, parentDiv, isMultiCursor, _cssName, keyboardType){
		...
		if(useTalkBack) {
			...
			//Biz Custom_키패드 처리 위한 분기 처리
			if (keyboardType === "qwerty_crt") {
			// Native 방식
			    innerDiv1.style.cssText = "width:560px; height:0px; margin-left:10px; margin-top:-2px;";
			} else {
			    // HTML 방식 (qwerty 등)
...
Transkey.prototype.setKeyboard = function(inputObj, isMultiCursor, useButton, useTranskey){
	...
	if(keyboardType=="qwerty"){
		...
		keyType="lower";
	}else if(keyboardType=="qwerty_crt"){ // bwlim
			this.setQwertyLayout(inputObj.id, div, isMultiC, _cssName, keyboardType);
			keyType="lower";
	}else{
