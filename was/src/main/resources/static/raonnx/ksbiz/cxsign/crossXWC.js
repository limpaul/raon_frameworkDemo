(function( win ) {
		...
		( function reCurrChecker(count) {
			if( isExistFeedback( onLoadFeedbackID ) ) {
				...
				try {
					...
					log("reCurrChecker sendMessage error", e);
				}
	...
	function sendMessage(message) {
		...
		if( CrossMessageSupport ) {
			...
			if( !document.getElementById( "XWC_service_iframe" ) ) {
				...
				div.innerHTML = "<iframe seamless style='position: absolute; border: 0px; margin:0px; padding:0px; width:0px; height:0px;' id='XWC_service_iframe' src='" + saveCertDomain + saveCertUrl + "' marginheight='0' marginwidth='0' frameborder='0' title='XWC_service_iframe'></iframe>";
	...
	*/
	...
	XWC.hashPlainText = crossInvoke.bind(this, "hashPlainText");
	XWC.signSmartUsim = crossInvoke.bind(this, "signSmartUsim");
