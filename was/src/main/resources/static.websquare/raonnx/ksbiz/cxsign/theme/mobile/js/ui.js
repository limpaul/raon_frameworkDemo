...
var ui = {
	...
	init : function(show_area) {
		...
		for(var i=0; i<show_area.length; i++) {
			...
			//202103 : 탭 메뉴 display 설정
			if(str == ".navCertTab"){
				if($(".navCertTab").css("display") == "none"){
					$(".navCertTab").css("display","block");
				}
			} else {
				if($(".navCertTab").css("display") != "none"){
					$(".navCertTab").css("display","none");
				}
			}
	...
	initSign : function(){
		$(".btn_back").off("click").on("click", function(){
			if ( KSBizConfig.navCertTabUse != undefined && KSBizConfig.navCertTabUse && (cxCtrl.isSignPage() || cxCtrl.isManageCertPage()) && KSBizConfig.navCertButtonCount != "2"){
				ui.initNavMenu();
			} else {
				uiCtrl.winClose();
			}
	...
	*/
	focus : function(location){
		...
		} else if(location == "initNavMenu") {
			var i;
			for(i =0; i<$(".navCertTab_lst a").length; i++){
				$(".navCertTab_lst a").eq(i).attr('tabindex', tidx++);
			}
		}
	...
	//가져오기 ui
	initImport : function(){
		parent.TouchEnNx.processingbar(false);
		cxsign.policyID = "ImportCert";
		$("#browserCertCheck_area_data1").attr("aria-label", uiCtrl.getLang("first_digits"));
		$("#browserCertCheck_area_data2").attr("aria-label", uiCtrl.getLang("back_digits"));
		...
		var show_area = ["browser_export_area_check",".info_rac"];
		ui.init(show_area);
		$("#browserCertCheck_area_data1").on("keydown", function(e){
			if(e.keyCode == 13){
				$("#browserCertCheck_area_data2").focus();
			}
		});
		$("#browserCertCheck_area_data2").on("keydown", function(e){
			if(e.keyCode == 13){
				$(this).blur();
			}
		});
	...
	initNavMenu : function(){
		$(".navCertTab_close").off("click").on("click", function(){
			uiCtrl.winClose();
		});
		var show_area = [".navCertTab"];
		ui.init(show_area);
		...
		mutations.forEach(function(mutation) {
			if (mutation.type === 'childList' && $(".navCertTab").is(":visible")) {
			setTimeout(function() {
				$("*").not(".navCertTab").blur();
				$(".navCertTab_lst li a").first().focus();
				focusLoop($(".btn_back"), $(".navCertTab_lst a:last"));
			}, 0);
			observer.disconnect();
			}
		});
		...
		ui.navActionEvent();
		// 포커스 설정
		ui.focus("initNavMenu");
	},

	//202103 : 버튼이 2개 일때는 default 리스트 클릭
	naveMenuDefaultClick : function(){
		...
		if( chkdefault == 0){
			...
			if (KSBizConfig.navCertTabDefault == "PUBLIC"){
				$(".publicCert").click();
			} else if(KSBizConfig.navCertTabDefault == "YESSIGN" ) {
				$(".yessignFinCert").click();
			} else if(KSBizConfig.navCertTabDefault == "YESSIGNCORP") {
				$(".yessignFinCertCorp").click();
			} else if(KSBizConfig.navCertTabDefault == "YESSIGNINT"){
				$(".yessignInt").click();
			} else {
				$(".publicCert").click();
			}
	...
	// 202103 : 탭 메뉴 리스트 구성
	navCertTabSet : function(){
		var tabList = KSBizConfig.navCertTabList.split("|");
		$(".navCertTab .navCertTab_lst").empty();
		var ulobj = $("<ul>");
		for(var i in tabList){
			...
			if(tabList[i] == "PUBLIC"){
				spanobj.html(uiCtrl.getLang("public_certifition_mobile"));
			} else if(tabList[i] == "YESSIGN"){
				spanobj.html(uiCtrl.getLang("yessignFinCert_certifition_mobile"));
			} else if(tabList[i] == "YESSIGNCORP"){
				spanobj.html(uiCtrl.getLang("yessignFinCorpCert_certifition_mobile"));
			...
			liobj.append(aobj);
			ulobj.append(liobj);
			var navContent = $(".navCertTab .navCertTab_lst");
			navContent.append(ulobj);
		}	
	},

	// 202103 : 탭 메뉴 이벤트 설정 
	navActionEvent : function(){
		$(".navCertTab .navCertTab_lst ul li a").each(function(i){
			// $(this).off("click").on("click", function(e){
			// 	var funcName = cxCtrl.gettNavMenuavActionBtnFunc($(this)[0].className.split(" ")[0]);
			// 	ui[funcName]();
			// });
			var aobj = $(this);
			$(this).parent().off("click").on("click", function(e){
				$(".navCertTab .navCertTab_lst ul li").removeClass("active_color");
	...
	// 202103 : 금융결제원 화면
	initYessign : function(){
		$("#header").show();
		$("html").css("background","#fff");
		$("body").css("background","#fff");
		$(".btn_back").off("click").on("click", function(){
			if ( KSBizConfig.navCertTabUse != undefined && KSBizConfig.navCertTabUse){
				ui.initNavMenu();
			} else {
				uiCtrl.winClose();
			}
		});
		var show_area = ["yessignFinCert_area"];
		ui.init(show_area);
		$(".top_tit_area .title .lang").eq(0).html("");
		$(".top_tit_area .title .lang").eq(0).html(uiCtrl.getLang("yessignFinIntCert_certifition"));
		// 금융결제원 스크립트 로딩
		$(this).loadYessignFinCert();
		$(".yessignFinCert_area .btn_area .btn_blu").off("click").on("click", function(){
			ui.executeYessign();
		});
	},
	...
	pwdCheckUi : function(){
		if(cxCtrl.isExportCertPage()){
			...
			$(".password_area > .btn_area > .btn_blu").off("click").on("click", function(e){
				$(this).exportCert();
			});
			$(".password_area > .btn_area > .btn_gry").off("click").on("click", ui.initExport);
		} else if(cxCtrl.isImportCertPage()){
			$(".btn_back").off("click").on("click", ui.initImport);
			$(".password_area > .btn_area > .btn_blu").off("click").on("click", function(e){
				$(this).importCert();
			});
			$(".password_area > .btn_area > .btn_gry").off("click").on("click", ui.initImport);
					...
					if(result.length > 0){
						...
						DC_get(KSBizConfig.browserCertExportUrl, exportReqestData, function(res){
							if(res != ""){
								...
								$("#browserCertCheck_area_data1").attr('aria-label', uiCtrl.getLang("first_digits") + first_data);
								$("#browserCertCheck_area_data2").attr('aria-label', uiCtrl.getLang("back_digits") + second_data);
...
$.fn.yessignFinCertManage = function(){
	KSBizConfig.yessignFinCert.mode.Sdk.manage({
		...
		fail: function(error) {
			...
			errors.status = error.code;
			errors.message = error.message;
			if(typeof KSBizConfig.navCertButtonCount != "undefined" && KSBizConfig.navCertButtonCount == 1){
	...
	if (signFormat.type == "PKCS1"){
		for (var i=0 ; i<ArrPlainText.length; i++) {
			if(cxCtrl.getOptions().encoding == "euckr"){
				if(typeof iniURLEncode != "undefined"){
	...
	if(chkUcpidDataSign){
		...
		KSBizConfig.yessignFinCert.mode.Sdk.sign({
			...
			success: function(result) {
				...
				data.signedData = signedData;
				if (signFormat.type == "PKCS1"){ 
					data.rawData = content.binary.binaries[0];
				}
				if(result.rValue) {
					var element = result.rValue;
					data.vidRandom = element;
