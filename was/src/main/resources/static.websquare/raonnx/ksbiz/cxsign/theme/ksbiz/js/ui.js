...
$(function(){
	...
	// 슬라이드 도움말 버튼 설정
	$(".slide_help_area .slide_btn").on("click", function(){
		if($(".slide_help_area").hasClass("hidden") == true){
			$(".slide_help_area").removeClass("hidden");
		} else {
			$(".slide_help_area").addClass("hidden");
		}
	});
	// 저장소 html 초기화
	...
	$(".issue_area .pin_area .btn_org").on("click", ui.issueInputPin);
		$(".yessignFinCert_area .btn_area .yessignFinCert_btn").on("click", function(){
		$(this).loadYessignFinCert();
	});
	// 비밀번호 엔터
...
var ui = {
	...
	setHelp : function(lang){
		...
		DC_get("help_layer_" + lang + ".html", "", function(res){
			...
			// 슬라이드 영역의 public 에 공동인증서 관련 도움말 추가
			$(".slide_help_area .content .public").html(res);
			$(".help-layer-btn, .help-btn").on("click", function(){
				var helpWin = window.open("about:blank", "pop_ksbiz_help", "directories=no,location=no,menubar=no,titlebar=no,status=no,scrollbars=yes,resizable=yes,width=720px,height=800px");
				if(helpWin == null){
	...
	initSign : function(){
		...
		$(".manage_area").hide();
		$(".init_area").hide();
		$(".yessignFinCert_area").hide();
		$(".top_tit_area .title .lang").eq(0).html(uiCtrl.getLang("sign_dialog_title"));
		// navTabMenu 사용시 로고박스 가리기
		if (typeof KSBizConfig.navCertTabUse != "undefined" && KSBizConfig.navCertTabUse && cxCtrl.isSignPage()) {
			$(".logo_box").hide();
		}
		$(".storage_area").show();
		...
		//브라우저 인증서 사용안내 alt 내용 추가
		$(".help-layer .image_title").attr("alt", uiCtrl.getLang("help_layer_image_title"));

		// 인증서 암호 입력창 title 설정
		var langText = uiCtrl.getLang("new_info_for_else");
		$("#certpwd").attr("title",langText);
	},
	initManage : function(){
		$(".logo_box").hide();
		$(".bank_info").hide();
	...
	initNavCertMenu : function(){
		$("#container").css("border-left","0px");
		$(".navCertTab").show();
		if(cxCtrl.isManageCertPage()){
			$(".navCertTab").height(563);
		}

		//  KSBizConfig.navCertTabLis 에 들어가는 리스트 추가
		var tabList = KSBizConfig.navCertTabList.split("|");
		var ulobj = $("<ul>");
		for(var i in tabList){
			var liobj = $("<li>");
			var aobj = $("<a href='#'>").addClass(CXCONST.NAVMENU[tabList[i]]);
			var iconobj = $("<div>");
			iconobj.addClass("iconWrap");
			var spanobj = $("<span>");

			if(tabList[i] == "PUBLIC"){
				spanobj.html(uiCtrl.getLang("public_certifition_web"));
			} else if(tabList[i] == "YESSIGN"){
				spanobj.html(uiCtrl.getLang("yessignFinCert_certifition"));
			} else if (tabList[i] == "YESSIGNCORP"){
				spanobj.html(uiCtrl.getLang("yessignFinCorpCert_certifition"));
			} else if (tabList[i] == "YESSIGNINT"){
				spanobj.html(uiCtrl.getLang("yessignFinIntCert_certifition"));
			} else {
				spanobj.html(tabList[i]);
			}
			// 리스트마다 들어가는 이미지가 다르기 때문에 해당 부분은 class로 처리함
			aobj.addClass(tabList[i].toLowerCase());
			aobj.append(iconobj);
			aobj.append(spanobj);
			liobj.append(aobj);
			ulobj.append(liobj);
		}
		var navContent = $(".navCertTab .content");
		navContent.append(ulobj);
		ui.navCertMenuActionEvent();

	},
	// navCertMenu에 이벤트 추가
	navCertMenuActionEvent: function(){
		$(".navCertTab .content ul li a").each(function(i){
			var aobj = $(this);
			$(this).parent().off("click").on("click", function(e){
				$(".navCertTab .content ul li").removeClass("active_color");
				var funcName = cxCtrl.gettNavMenuavActionBtnFunc(aobj[0].className.split(" ")[0]);
				$(this).addClass("active_color");
				cxsign.selectTabMenu = cxCtrl.getNavMenuConfig(aobj[0].className.split(" ")[0]);
				ui[funcName]();
			});
		});
	},
	// 탭 사이즈 맞추기 
	navCertMenuSize : function () {
		// 탭 길이와 도움말 길이가 전자서명길이보다 작을때 조절해주기
		var singHeight = Number($("#container").css("padding-bottom").split("px")[0]) + $("#container").height();
		var tabHeight = $(".navCertTab").height();
		if(singHeight != tabHeight){
			if(TOUCHENEX_UTIL.isIE() && cxCtrl.isManageCertPage()){
				singHeight -= 1;
			}
			$(".navCertTab").css("height",singHeight+"px");
		}
	},
	// 전자서명과 인증서관리의 초기 창 -> navCertTabDefault값을 보고 클릭 
	initBlock : function(){
		$(".password_area").hide();
		$(".storage_area").hide();
		$(".certification_area").hide();
		...
		$(".init_area").show();
		if(cxCtrl.isManageCertPage()){
			$(".init_area").css('height',478);
			ui.setWinPosition(543);
		} else if(cxCtrl.isSignPage()){
			$(".init_area").css('height',497);
			ui.setWinPosition(655);
		}
		if(typeof KSBizConfig.navCertTabUse != "undefined" && KSBizConfig.navCertTabUse) {
			if(typeof KSBizConfig.navCertTabDefault != "undefined" && KSBizConfig.navCertTabDefault){
				if(typeof cxCtrl.getOptions() != "undefined" && typeof cxCtrl.getOptions().navCertTabDefault != "undefined") {
					KSBizConfig.navCertTabDefault = cxCtrl.getOptions().navCertTabDefault;
				}
				var navCertTabLists = KSBizConfig.navCertTabList.split("|");
				var chkdefault = 0;
				for (var i in navCertTabLists){
					if(navCertTabLists[i] == KSBizConfig.navCertTabDefault) {
						chkdefault++;
					}
				}
				if( chkdefault == 0){
					var error = {};
					error.message = "탭 기본값을 확인하세요."
					uiCtrl.errorAlert(error, function(){uiCtrl.winClose()});
				}else {
					$("."+CXCONST.NAVMENU[KSBizConfig.navCertTabDefault]).click();
				}
			} else {
				$("."+CXCONST.NAVMENU.PUBLIC).click();
			}
		} else {
			ui.initPublic();
		}
	...
	initPublic : function(){
		cxsign.navTabID = "public";
		$(".top_tit_area .accessbility_hidden").text(uiCtrl.getLang("public_certifition_mobile"));
		ui.initPopClose();
		$("#container").css("height","auto");
		if(cxCtrl.isSignPage()){
			$(".slide_help_area .content .public").show();
			// 네비게이션 탭 데이터 전자서명 사이즈 조절
			if(typeof cxCtrl.getOptions().format != "undefined"){
				if(typeof cxCtrl.getOptions().confirmSignTitleImage != "undefined"){
					$(".navCertTab").css("height","764");
				} else {
					$(".navCertTab").css("height","694");
				}
			} else if(typeof cxCtrl.getOptions().confirmSignTitleImage != "undefined"){
				$(".navCertTab").css("height","652");
			}
			// 도움말이 닫혀있는 경우 열어주기
			if($(".slide_help_area").hasClass("hidden")) {
				$(".slide_help_area").removeClass("hidden");
			}
			ui.initSign();
	...
	initYessignFinCert : function(){
		cxsign.navTabID = "yessign";
		ui.initPopClose();
		$(".account_info").hide();
		$(".bank_info").hide();
		$(".password_area").hide();
		...
		$(".init_area").hide();
		$(".help-layer").hide();
		$("#certpwd_layout").css('display','none');
		$("#container").css("height","auto");
		$(".slide_help_area").hide();
		$(".yessignFinCert_area").show();
		if(cxCtrl.isManageCertPage()){
			$(".yessignFinCert_area").css('height',365);
		} else if(cxCtrl.isSignPage()){
			$(".yessignFinCert_area").css('height',384);
		}
		ui.navCertMenuSize();
		ui.focus();
		$(".yessignFinCert_area .btn_area .yessignFinCert_btn").click();

		if(cxsign.selectTabMenu === 'YESSIGN'){
			$(".top_tit_area .accessbility_hidden").text(uiCtrl.getLang("yessignFinCert_certifition_mobile"));
		 } else if(cxsign.selectTabMenu === 'YESSIGNCORP') {
			$(".top_tit_area .accessbility_hidden").text(uiCtrl.getLang("yessignFinCorpCert_certifition_mobile"));
		 } else if(cxsign.selectTabMenu === 'YESSIGNINT') {
			$(".top_tit_area .accessbility_hidden").text(uiCtrl.getLang("yessignFinIntCert_certifition"));
		 }
	},
	viewAccountInfo : function(dataArr){
		...
		for(var i = 0; i < dataArr.length; i++){
			...
			var thObj = $("<th>").text(data.name);
			var tdObj = $("<td>").text(data.value);
	...
	initGrid : function(){
		...
		//grid title 추가 - 웹접근성
		$('#grid_grid_focus').attr("title", "grid_grid_focus");
		$('#grid_grid_search_all').attr("title", "grid_grid_search_all");
		...
		w2ui.grid.on('click', function(event) {
			...
			if(cxsign.firstCert){
				...
				if(gridList[_idx-1].expire_status == "EXPIRE") {
					$('#grid_grid_focus').val(uiCtrl.getLang("select_certificate_voice_msg") + gridList[_idx-1].type + gridList[_idx-1].user_name + gridList[_idx-1].after_date + gridList[_idx-1].org  +  uiCtrl.getLang("select_certificate_voice_select_msg") + uiCtrl.getLang("cert_v_ERR_CERT_HAS_EXPIRED"));
					$('#grid_grid_focus').attr('aria-label', uiCtrl.getLang("select_certificate_voice_msg") + gridList[_idx-1].type + gridList[_idx-1].user_name + gridList[_idx-1].after_date + gridList[_idx-1].org  +  uiCtrl.getLang("select_certificate_voice_select_msg") + uiCtrl.getLang("cert_v_ERR_CERT_HAS_EXPIRED"));
				} else if(gridList[_idx-1].expire_status == "BEFOREEXPIREMONTH"){
					$('#grid_grid_focus').val(uiCtrl.getLang("select_certificate_voice_msg") + gridList[_idx-1].type + gridList[_idx-1].user_name + gridList[_idx-1].after_date + gridList[_idx-1].org  +  uiCtrl.getLang("select_certificate_voice_select_msg") + uiCtrl.getLang("cert_expire_warning_message").replace("%s", gridList[_idx-1].after_date).replace("<br>","").replace("<br>",""));
					$('#grid_grid_focus').attr('aria-label', uiCtrl.getLang("select_certificate_voice_msg") + gridList[_idx-1].type + gridList[_idx-1].user_name + gridList[_idx-1].after_date + gridList[_idx-1].org  +  uiCtrl.getLang("select_certificate_voice_select_msg") + uiCtrl.getLang("cert_expire_warning_message").replace("%s", gridList[_idx-1].after_date).replace("<br>","").replace("<br>",""));
				} else{
				...
				if(gridList[_idx-1].expire_status == "EXPIRE") {
					$('#grid_grid_focus').val(gridList[_idx-1].type + gridList[_idx-1].user_name + gridList[_idx-1].after_date + gridList[_idx-1].org  + uiCtrl.getLang("select_certificate_voice_select_msg") + uiCtrl.getLang("cert_v_ERR_CERT_HAS_EXPIRED"));
					$('#grid_grid_focus').attr('aria-label', gridList[_idx-1].type + gridList[_idx-1].user_name + gridList[_idx-1].after_date + gridList[_idx-1].org  + uiCtrl.getLang("select_certificate_voice_select_msg") + uiCtrl.getLang("cert_v_ERR_CERT_HAS_EXPIRED"));
				} else if(gridList[_idx-1].expire_status == "BEFOREEXPIREMONTH"){
					$('#grid_grid_focus').val(gridList[_idx-1].type + gridList[_idx-1].user_name + gridList[_idx-1].after_date + gridList[_idx-1].org  + uiCtrl.getLang("select_certificate_voice_select_msg") + uiCtrl.getLang("cert_expire_warning_message").replace("%s", gridList[_idx-1].after_date).replace("<br>","").replace("<br>",""));
					$('#grid_grid_focus').attr('aria-label', gridList[_idx-1].type + gridList[_idx-1].user_name + gridList[_idx-1].after_date + gridList[_idx-1].org  + uiCtrl.getLang("select_certificate_voice_select_msg") + uiCtrl.getLang("cert_expire_warning_message").replace("%s", gridList[_idx-1].after_date).replace("<br>","").replace("<br>",""));
				} else{
			...
			// 20240418 포커스 이동시켰다가 돌아오게하기
			if ($('#grid_grid_focus').is(':focus')) {
				var tempDiv = $('<div>'); // 임시 div 요소 생성
				tempDiv.attr('tabindex', '0'); // 포커스를 받을 수 있도록 tabindex 설정
				tempDiv.appendTo('body'); // 문서에 임시 div 추가
				tempDiv.focus(); // 임시 div로 포커스 이동
				setTimeout(function(){
					$('#grid_grid_focus').focus();
					tempDiv.remove(); // 임시 div 삭제
				},0);
			}
	...
	focus : function(){
		/*
		// 인증서 암호 입력창 title 설정
		$('.add_title input').on("focus", function(){
			var langText = uiCtrl.getLang("new_info_for_else");
			$(this).attr("title",langText);
		});
		// a focus시 title 설정, blur시 title제거
		// $('a').on("focus", function(){
		// 	$(this).attr("title","선택됨");
		// });
		// $('a').on("blur", function(){
		...
		*/
		...
		if(cxCtrl.isSignPage() || cxCtrl.isUpdateCertPage()){
			...
			if(typeof KSBizConfig.navCertTabUse != "undefined" &&  KSBizConfig.navCertTabUse) {
				$(".navCertTab .content ul li a").eq(0).addClass("start_focus");
			} else {
				//$("#browser").find("a").eq(0).addClass("start_focus");
				$(".top_tit_area .btn_close").addClass("start_focus");
			}			

			if(cxsign.navTabID == "yessign"){
				$(".yessignFinCert_area .btn_area .btn_org").addClass("end_focus");
				$(".yessignFinCert_area .btn_area .btn_org").focus();
			} else {
				// end_focus 설정
				if(typeof KSBizConfig.browsercert_guide != "undefined" && KSBizConfig.browsercert_guide && cxsign.currentStorage == "browser") {
					if(typeof KSBizConfig.navCertTabUse != "undefined" &&  KSBizConfig.navCertTabUse) {
						if($(".password_area .btn_area .btn_wht2").hasClass("end_focus") == true) {
							$(".password_area .btn_area .btn_wht2").removeClass("end_focus");
							$(".password_area .btn_area .btn_wht2").off("keydown");
						}
						$(".slide_help_area .slide_btn_area .slide_btn").addClass("end_focus");
					} else {
						if($(".password_area .btn_area .btn_wht2").hasClass("end_focus") == true) {
							$(".password_area .btn_area .btn_wht2").removeClass("end_focus");
							$(".password_area .btn_area .btn_wht2").off("keydown");
						}
						$(".help-layer .content .help-layer-btn").addClass("end_focus");
					}
				} else {
					if($(".help-layer .content .help-layer-btn").hasClass("end_focus") == true) {
						$(".help-layer .content .help-layer-btn").removeClass("end_focus");
						$(".help-layer .content .help-layer-btn").off("keydown"); 
					}
					if($(".slide_help_area .slide_btn_area .slide_btn").hasClass("end_focus") == true) {
						$(".slide_help_area .slide_btn_area .slide_btn").removeClass("end_focus");
						$(".slide_help_area .slide_btn_area .slide_btn").off("keydown"); 
					}
			...
			$("#cmp_browser").find("a").eq(0).addClass("start_focus");
	...
	popPwdChange : function(callback){
		$(".pop_change .btn_org").off("click").on("click", function(e){
			...
			if($("#pop_change_new").val().length < KSBizConfig.certPwdDefaultRule.length){
				ui.alert(uiCtrl.getLang("short_new_cmp_password"));
	...
	clickToggle : function(keyBtn){
		ui.pastFocus = keyBtn.offsetParent;
		var obj;
		...
		if(isChecked){
			...
			if(transkey[id].inputObj.readOnly == false){
				transkey[id].inputObj.removeAttribute("tabindex");
			} else {
				transkey[id].inputObj.setAttribute("tabindex", "-1");
			}
		...
		if(cxCtrl.isSignPage() || cxCtrl.isManageCertPage() || cxCtrl.isUpdateCertPage()|| cxCtrl.isExportCertPage()){
			...
			$(".storage_more li").each(function(i){
				...
				try {
					...
					ksglo("storageVitalize.storage_more error",e);
				}
			});
		}
		// 인증서 복사 스토리지
		if(cxCtrl.isManageCertPage()){
			$(".pop_copy .storage_lst li").each(function(i){
				...
				try {
					...
					kslog("storageVitalize.isManageCertPage error",e);
				}
			});
		}
		// 인증서 발급 스토리지
		if(cxCtrl.isIssueCertPage()){
			$(".issue_area .storage_lst li").each(function(i){
				...
				try {
					...
					kslog("storageVitalize.isIssueCertPage error",e);
				}
			});
		}
	},
	...
	sign : function(){
		if(!cxCtrl.isCurrentCert()){
			ui.alert(uiCtrl.getLang("select_cert"), function(){
				ui.selectFocus();
			});
			return;
		}
		...
		ui.popPwdInput(false, function(isSave, encPwdObj){
			...
			if(storage == CXCONST.STORAGE.BROWSER){
				...
				if(storage == CXCONST.STORAGE.HSM
					...
					clientCtrl.importCert(storage, drive, certType, certData, pwdStr, pwdType, "", "", options, certImportCertCallback);
				}
			}
		});
	},
	issueSelectStorage : function(){
		...
		if(selectDrive.storage && selectDrive.storage){
			...
			ui.alert(uiCtrl.getLang("select_certstore_msg"), function(){
				ui.selectFocus();
			});
		}
	},
	issueInputPassword : function(){
		...
		if($("#issuepwd").val() == ""){
			ui.alert(uiCtrl.getLang("new_password_input"), function(){
				ui.selectFocus();
			});
			return;
		}
		if($("#issuepwd").val().length < KSBizConfig.certPwdCMPRule.length){
			ui.alert(uiCtrl.getLang("short_new_cmp_password"), function(){
				ui.selectFocus();
			});
			$("#issueFrm")[0].reset();
			return;
	...
	// 스마트인증 무설치
	loadUsim : function(){
		//여기서 json만들어서 보낼지.. signSmartUsim 만들지..
		if(document.getElementById("smartcertnpdiv")){
			$(this).smartUsim();
		} else {
			$.getScript(KSBizConfig.smartUsim.raon.src)
			.done(function(){
				$(this).smartUsim();
			})
			.fail(function(){
				ui.errorAlert("스마트 유심 로딩 실패");
			});
		}			
	},
	//이전 focus 설정 
	selectFocus : function(){
		if(!ui.pastFocus && ui.contentsFocus){
			ui.contentsFocus.focus();
			ui.contentsFocus = null;
		} else if(ui.pastFocus) {
			ui.pastFocus.focus();
			ui.pastFocus = null;
		}
	},
	executeYessign : function(){
		if (cxCtrl.isManageCertPage()) {
			$(this).yessignFinCertManage();
		} else if(cxCtrl.isSignPage()) {
			$(this).yessignFinCertSign();
		}
	}
};
...
$.fn.storageClick = function(storage, evt){
	...
	// 선택됨 aria-lable 적용
	$(".storage_lst li").find('a').removeAttr("aria-label");
	$(".storage_more li").find('a').removeAttr("aria-label");
	var innerText = storageObj.find('span.lang').text();
	storageObj.find('a').attr("aria-label", innerText + " 선택됨");
					...
					if(diskList.drives.length > 0){
						...
						for(var i in diskList.drives){
							...
							liTag.on("click", {
								...
								} else if(KSBizConfig.smartUsim != "undefined" && KSBizConfig.smartUsim.use && storage == CXCONST.STORAGE.PHONE && driveName == "Mobile_USIMsmartCERT"){
									ui.loadUsim();
								} else {
	...
	} else if(KSBizConfig.smartUsim != "undefined" && KSBizConfig.smartUsim.use && CXCONST.STORAGE.USIM == storage){
		// 스마트인증 무설치
		ui.loadUsim();
	} else{
		...
		if(typeof certList.installed && certList.installed == "FALSE"){
			ui.confirm(uiCtrl.getLang("usim_driver_install_after_close_confirm"), function(){
				...
				if(moduleWin == null){
					...
					moduleWin.location.href = KSBizConfig.usim.dream.download;
					if(cxCtrl.isManageCertPage()){
						uiCtrl.winClose();
					} else {
						uiCtrl.errorClose({
...
$.fn.viewCertDetail = function(){
	...
	viewCertDetailCallback = function(certDetail){
		...
		var makeDetailTrTxt = function(name, value){
			var trTxt = $("<tr>");
			var nameTxt = $("<th>").addClass("brln").html(name);
			var valTxt = "";
			if(value) {
		...
		ui.confirm(uiCtrl.getLang("delete_certificate_confirm"), function(){
			...
			if(CXCONST.STORAGE.BROWSER == cert.storage){
				...
				if(cert.storage == CXCONST.STORAGE.HSM
					...
					clientCtrl.deleteDiskCert(cert.storage, cert.drive, cert.cid, "", "", deleteDiskCertCallback);
				}
			}
		});
	} else {
					...
					var certExportCallback = function(certData){
						...
						if(tStorage == CXCONST.STORAGE.BROWSER){
							...
							if(tStorage == CXCONST.STORAGE.HSM
								...
								clientCtrl.importCert(tStorage, tDrive, certType, certData, pwdStr, pwdType, "", "", options, certCopyCallback);
							}
						}
					};
		...
		DC_get(KSBizConfig.browserCertExportUrl, importReqestData, function(res){
			if(res != ""){
				...
				if(res.status == CXCONST.CERTSYNC.STATUS.UP){
					...
					ui.alert(uiCtrl.getLang("cant_export_number"), function(){
						ui.selectFocus();
					});
					if(cxsign.browserCertInfo.cert != ""){
						cxsign.browserCertInfo.cert= "";
...
//무설치 스마트 유심 화면
$.fn.smartUsim = function() {
	var smartUsim = {
		msgType : KSBizConfig.smartUsim.msgType,
		msgIntegrity : "",
		plainText : cxCtrl.getPlainText(),
		signOrgView : false,
		...
		validate : false,
		randomChk : true,
		sitecode : KSBizConfig.smartUsim.raon.sitecode,
		modcode : KSBizConfig.smartUsim.raon.modcode,
		sourceData : ""
	};

	var smartUsimCallback = function(callback){
		 if(callback.hashedData != undefined && callback.sourceData != undefined && callback.signTime != undefined){
			smartUsim.signTime = callback.signTime;
			smartUsim.sourceData = callback.sourceData;
			kslog("smartUsim plainTextCallback",JSON.stringify(callback));
			//option 확인 및 KSBizConfig확인	
			if(smartUsim.msgType == "originHash"){
				smartUsim.msgIntegrity = {
					plainText : smartUsim.plainText,
					signOrgView : smartUsim.signOrgView,
				}
			} else {
				smartUsim.msgIntegrity = {
					plainText : smartUsim.plainText,
					signTime : smartUsim.signTime,
					signOrgView : smartUsim.signOrgView,
				}
			}
			...
			var options = cxCtrl.getOptions();
			if(options){
				if(options.issuerCertFilter != undefined && options.issuerCertFilter != ""){
					smartUsim.issuer = options.issuerCertFilter;
				} else {
					if(KSBizConfig.issuerCertFilter != ""){
						smartUsim.issuer = KSBizConfig.issuerCertFilter;
					} else{
						smartUsim.issuer = "";
					}
				}
				if(options.subjectCertFilter != undefined && options.subjectCertFilter != ""){
					var subject = cxCtrl.getOptions().subjectCertFilter.split("=");
					smartUsim.subject = subject[1];
				} else {
					smartUsim.subject = "";
				}
				if(options.policyOidCertFilter != undefined && options.policyOidCertFilter != ""){
					smartUsim.oid = options.policyOidCertFilter;
				} else {
					if(KSBizConfig.policyOidCertFilter != ""){
						smartUsim.oid = KSBizConfig.policyOidCertFilter;
					} else{
						smartUsim.oid = "";
					}
				}
				if(options.disableExpireFilter != undefined && options.disableExpireFilter == false){
					smartUsim.validate = true;
				} else {
					if(KSBizConfig.disableExpireFilter == false){
						smartUsim.validate = true;
					} else{
						smartUsim.validate = false;
					}
				}
			}
			var reqMsg = { msg : callback.hashedData };
			smartUsim.msg = JSON.stringify(reqMsg);
			$(this).signSmartUsim(smartUsim);
		} else {
			kslog("smartUsim.smartUsimCallback callback error");
		}
	};
	browserCtrl.hashPlainText(cxCtrl.getPlainText(), smartUsim.msgType, smartUsimCallback);

}
$.fn.signSmartUsim = function(smartUsim){
	var sign = jSmartCertNP.Sign({
		msgType : smartUsim.msgType,
		msg : smartUsim.msg,
		msgIntegrity :  JSON.stringify(smartUsim.msgIntegrity),
		signedDataType : smartUsim.signedDataType,
		sitecode : smartUsim.sitecode,
		modcode : smartUsim.modcode,
		subject : smartUsim.subject,
		issuer : smartUsim.issuer,
		oid : smartUsim.oid,
		serial : smartUsim.serial,
		validate : smartUsim.validate,
		randomChk : smartUsim.randomChk,
		complete : function(resObj){
			var response = JSON.parse(resObj);
			kslog("signSmartUsim.signSmartUsimcallback complete response", response);
			if(response.returnObj != undefined && response.returnObj.returnCode == "0" ){
				var signResult = response.signResult;
				var b64SignedData = signResult.b64SignedData[0];
				var b64Signer = signResult.b64Signer;
				//임시로 cxsign.random.....
				var random= signResult.R[0];
				var hashedData = JSON.parse(smartUsim.msg).msg;
				var plain = smartUsim.plainText;
				var sourceData = smartUsim.sourceData;
				var msgType = smartUsim.msgType
				var signSmartUsimcallback = function(callback){
					if(callback != undefined){
						kslog("signSmartUsim.signSmartUsimcallback callback", callback);
						callback.vidRandom = random;
						uiCtrl.callbackClose(callback);
					}
				}
				browserCtrl.signSmartUsim(b64SignedData, b64Signer, hashedData, plain, sourceData, msgType, signSmartUsimcallback);
			} else if(response.returnObj != undefined){
				ui.loading.end();
				ui.alert(response.returnObj.returnMsg, function(){
					ui.storageClick(KSBizConfig.initStorage);
				});
			}
		}
...
// 금융인증서 load script
$.fn.loadYessignFinCert = function(){
	// 어떤 금융인증서인지에 따라 주소 세팅
	if(cxsign.selectTabMenu === 'YESSIGN'){
		KSBizConfig.yessignFinCert.src = KSBizConfig.yessignSrc.finCertSrc;
	} else if(cxsign.selectTabMenu === 'YESSIGNCORP') {
		KSBizConfig.yessignFinCert.src = KSBizConfig.yessignSrc.corpFinCertSrc;
	} else if(cxsign.selectTabMenu === 'YESSIGNINT') {
		KSBizConfig.yessignFinCert.src = KSBizConfig.yessignSrc.intFinCertSrc;
	}
	var today = new Date();
	...
	today = String(today.getFullYear()) + String(today.getMonth()+1) + today_date + String(today.getHours()) + String(today.getMinutes() + String(today.getMilliseconds()));
	kslog("loadYessignFinCert.loadDate", today);
	if((cxsign.selectTabMenu === 'YESSIGN' && typeof FinCert !== 'undefined') || 
		(cxsign.selectTabMenu === 'YESSIGNCORP' && typeof FinCertCorp !== 'undefined') ||
		(cxsign.selectTabMenu === 'YESSIGNINT' && typeof FinCertInt !== 'undefined')) {
		$(this).initYessignFinCert();
	} else {
		$.getScript(KSBizConfig.yessignFinCert.src + "?dt" + $(this).getDate())
		.done(function(){
			$(this).initYessignFinCert();
		})
		.fail(function(){
			ui.errorAlert("금융인증서 로딩 실패");
		});
	}
}
// 금융결제원 init 과정
$.fn.initYessignFinCert = function() {
    var param = {};
    param.orgCode = KSBizConfig.yessignFinCert.orgCode;
    param.apiKey = KSBizConfig.yessignFinCert.apiKey;
    param.lang = KSBizConfig.yessignFinCert.lang;

    if (KSBizConfig.yessignFinCert.clientOrigin.length > 0) {
      param.clientOrigin = KSBizConfig.yessignFinCert.clientOrigin;
    }
    if (KSBizConfig.yessignFinCert.uniqVal.length > 0) {
      param.uniqVal = KSBizConfig.yessignFinCert.uniqVal;
    }

	// 금융인증서 값에 따라 mode값 세팅
	if(cxsign.selectTabMenu === 'YESSIGN'){
		KSBizConfig.yessignFinCert.mode = FinCert;
	} else if(cxsign.selectTabMenu === 'YESSIGNCORP') {
		KSBizConfig.yessignFinCert.mode = FinCertCorp;
	} else if(cxsign.selectTabMenu === 'YESSIGNINT') {
		KSBizConfig.yessignFinCert.mode = FinCertInt;
	}

    if (JSON.stringify(param) === "{}") {
		kslog("initYessignFinCert.param", param);
    	KSBizConfig.yessignFinCert.mode.Sdk.init();
    } else {
      ...
      param.fail = function(error){
		//900001 : 미지원 브라우저 에러 
		errors = {}
		errors.status = error.code;
		errors.message = error.message;
		uiCtrl.errorAlert(errors);
	  };
	  KSBizConfig.yessignFinCert.mode.Sdk.init(param);
    }
}
// 금융결제원 인증서 관리
$.fn.yessignFinCertManage = function(){
	KSBizConfig.yessignFinCert.mode.Sdk.manage({
		success : function(){
			ui.alert("금융 인증 서비스가 종료됨");
		},
		fail: function(error) {
			kslog("yessignFinCertManage.sign_fail", error);
			errors = {}
			errors.status = error.code;
			errors.message = error.message;
			if(errors.status != "800000"){
				uiCtrl.errorAlert(errors);		
			} else {
				// set focus
				setTimeout(function(){
...
// 금융결제원 전자서명
$.fn.yessignFinCertSign = function() {
	// info.signType 설정
	var signType = "01";

	// signFormat 설정
	var signFormat = {};
	if(typeof cxCtrl.getOptions().signType != "undefined" && cxCtrl.getOptions().signType == "PKCS1"){
		KSBizConfig.yessignFinCert.signType = "PKCS1";
	}
	signFormat.type = KSBizConfig.yessignFinCert.signType;

	if (signFormat.type == "CMS"){
		signFormat.CMSInfo = {};
		if (cxCtrl.getOptions().ssn){
			signFormat.CMSInfo.ssn = cxCtrl.getPlainText().split("&")[0];
		} else {
	...
	// 원문 값 수정
	var ArrPlainText = [];
	var orPlainText = cxCtrl.getPlainText();
	var multiOrPlainText;
	var enableTextViewCheck = false;
	var content = {};
	if (cxCtrl.getOptions().multiSign){
		// 다중 전자서명인 경우
		multiOrPlainText = orPlainText.split(cxCtrl.getOptions().multiSignDelimiter);
		for (var i =0; i<multiOrPlainText.length; i++){
			ArrPlainText.push(multiOrPlainText[i]);
		}
	} else if (cxCtrl.getOptions().format){
		// 폼 전자서명인 경우
		enableTextViewCheck = true;
		orPlainText += "&__USER_CONFIRM_FORMAT=" + encodeURIComponent(cxCtrl.getOptions().format);
		ArrPlainText.push(orPlainText);

	} else {
		enableTextViewCheck = false;
		ArrPlainText.push(orPlainText);
	}
	//content 설정
	if (signFormat.type == "PKCS1"){
		for (var i=0 ; i<ArrPlainText.length; i++) {
			var encodePlainText = "";
			if(cxCtrl.getOptions().encoding == "euckr"){
				if(typeof JSTextEncoder != "undefined"){
					ArrPlainText[i] =  JSTextEncoder("euc-kr", {'NONSTANDARD_allowLegacyEncoding' : true}).encode( ArrPlainText[i] );
					ArrPlainText[i] = String.fromCharCode.apply(null, ArrPlainText[i]);
					ArrPlainText[i] = btoa(ArrPlainText[i]);
				}
			} else {
				encodePlainText = TOUCHENEX_UTIL.Base64.encode(ArrPlainText[i]);
				ArrPlainText[i] = encodePlainText;
			}
		}
		content.binary = {};
		content.binary.binaries = ArrPlainText;
	} else {
		content.plainText = {};
		content.plainText.plainTexts = ArrPlainText;
		if(typeof cxCtrl.getOptions().encoding != "undefined"){
			if(cxCtrl.getOptions().encoding == "euckr"){
				content.plainText.encoding = 'EUC-KR';
			} else {
				content.plainText.encoding = 'UTF-8';
			}
		} else {
			content.plainText.encoding = 'UTF-8';
		}
	}
	// ucpidInfo check and set
	var plainSignFormat = {};
	var plainSignContent = {};
	var plainSignType = {};
	var chkUcpidDataSign = false;
	if(cxCtrl.getOptions().signType == "ucpid" && cxCtrl.getOptions().ucpidInfo){
		if(content.plainText.plainTexts[0] != '') {
			chkUcpidDataSign = true;
			plainSignFormat = signFormat;
			plainSignContent = content;
			plainSignType = signType;
		}
		kslog("yessignFinCertSign.ucpidInfo force set");
		signFormat = {type: "CMS"};
		content = {ucpidInfo: cxCtrl.getOptions().ucpidInfo};
		signType = "12";
	}
	// 금융결제원 전자서명 실행
	kslog("yessignFinCertSign.sign_signFormat", signFormat);
	kslog("yessignFinCertSign.sign_Content", content);
	kslog("yessignFinCertSign.sign_algorithms", KSBizConfig.yessignFinCert.algorithms);
	kslog("yessignFinCertSign.info_signType", signType);

	// 20241016 : ucpid & data sign 조건인 경우 배열로 통신
	if(chkUcpidDataSign){
		KSBizConfig.yessignFinCert.mode.Sdk.sign([{
			content: content,
			info: {
				signType: signType
			},
			success: function(result) {
				kslog("yessignFinCertSign.sign_success", result);
				var data = {};
				data.filename = "";
				var sigedDataArr = [];
				var signedData = "";
				if(Array.isArray(result)){
					result.forEach(function(res){
						sigedDataArr = [];
						if(signedData != "") signedData += "::"
						res.signedVals.forEach(function(element){
							sigedDataArr.push(element);
						});
						signedData += sigedDataArr.join("|");
						if (signFormat.type == "PKCS1"){ 
							data.rawData =  TOUCHENEX_UTIL.Base64.encode(cxCtrl.getPlainText());
						}
						if(res.rValue) {
							var element = res.rValue;
							data.vidRandom = element;
						}
						if (res.certificate){
							pemCert="-----BEGIN CERTIFICATE-----\n";
							for(i=0; i<res.certificate.length; i+=64) 
								pemCert+=res.certificate.substr(i,64)+"\n";
			...
			fail: function(error) {
				kslog("yessignFinCertSign.sign_fail", error);
				errors = {}
				errors.status = error.code;
				errors.message = error.message;
				if(errors.status != "800000"){
			...
			signFormat:plainSignFormat,
			content: plainSignContent,
			algorithms: KSBizConfig.yessignFinCert.algorithms,
			view : {
