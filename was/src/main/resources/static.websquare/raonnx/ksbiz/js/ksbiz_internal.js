		...
		var resSignCallback = function(data, error){
			if(error){
				...
				if(data.signedData){
					...
					if(bucket.options.hashFileInfo){
						var AddValue = 
						base64ToHex(data.signedData) + getHexEncoded(encodeUTF8String(bucket.options.hashFileInfo.filepath)) + 
						getHexEncoded(encodeUTCTime(bucket.options.hashFileInfo.time));

						bucket.result.data = hexToBase64(AddValue);
					}
