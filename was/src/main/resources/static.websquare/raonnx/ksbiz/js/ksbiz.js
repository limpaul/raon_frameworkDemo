...
var KeySharpBiz = {
    ...
    // sign_ucpid.jsp
	ucpidDataSign : function(data, complete, options){
    	if(!options) {
			options = {ucpidInfo: {userAgreement: "default"}};
			options.ucpidInfo.userAgreeInfo = {realName: true, gender: true, nationalInfo: true, birthDate: true, ci: true};
		}

		if(typeof options.ucpidInfo  === "undefined"
			|| typeof options.ucpidInfo.userAgreement === "undefined"
			|| typeof options.ucpidInfo.userAgreeInfo === "undefined"
			|| typeof options.ucpidInfo.userAgreeInfo.realName === "undefined"
			|| typeof options.ucpidInfo.userAgreeInfo.gender === "undefined"
			|| typeof options.ucpidInfo.userAgreeInfo.nationalInfo === "undefined"
			|| typeof options.ucpidInfo.userAgreeInfo.birthDate === "undefined"
			|| typeof options.ucpidInfo.userAgreeInfo.ci === "undefined"){
			alert("check ucpidInfo agreement value");
			return;
		}

		DC_get(KSBizConfig.yessignFinCert.nonceUrl, "", function(nonce){
			if(nonce){
				options.ucpidInfo.ucpidNonce = encodeURIComponent(nonce);
				if(typeof complete == "function"){
					complete.complete = complete;
					complete.context = encodeURIComponent(nonce);
				} else if(typeof complete == "object"){
					if(complete.context){
						complete.context = complete.context + "||" + encodeURIComponent(nonce);
					} else {
						complete.context = encodeURIComponent(nonce);
					}
				}
			} else {
				alert("check ucpidInfo nonce value");
				return;
			}

			// option 설정
			options.ucpidInfo.ispUrlInfo = location.hostname;
			options.addNonce = false;
			options.signType = "ucpid";

			// config 설정 변경
			var configOrigin = {};
			configOrigin.yessignFinCert = {};
			configOrigin.uitype = KSBizConfig.uitype;
			configOrigin.navCertTabUse = KSBizConfig.navCertTabUse;
			configOrigin.navCertTabList = KSBizConfig.navCertTabList;
			configOrigin.navCertTabDefault = KSBizConfig.navCertTabDefault;
			configOrigin.navCertButtonCount = KSBizConfig.navCertButtonCount;
			configOrigin.yessignFinCert.signType = KSBizConfig.yessignFinCert.signType;
			configOrigin.yessignFinCert.algorithms = KSBizConfig.yessignFinCert.algorithms;

			KSBizConfig.uitype = "html";
			KSBizConfig.navCertTabUse = true;
			KSBizConfig.navCertTabList = "YESSIGN";
			KSBizConfig.navCertTabDefault = "YESSIGN";
			KSBizConfig.navCertButtonCount = "2";
			KSBizConfig.yessignFinCert.signType = "CMS";
			KSBizConfig.yessignFinCert.algorithms = "RSASSA-PKCS1-v1_5_SHA256";

			nativeBrowserCheck();
			if(KSBizConfig.uitype == "native"){
				alert("지원하지 않는 브라우저입니다. (ucpidSign not support)");
				return;
			}

			var callbackfn = {};
			if(typeof complete == "function"){
				callbackfn.complete = complete;
				callbackfn.context = options.ucpidInfo.ucpidNonce;
			} else if(typeof complete == "object"){
				callbackfn.complete = complete.complete;
				if(complete.context){
					callbackfn.context = complete.context + "||" + options.ucpidInfo.ucpidNonce;
				} else {
					callbackfn.context = options.ucpidInfo.ucpidNonce;
				}
			}
			completeCallback = callbackfn.complete;
			callbackfn.complete = function(result, context) {
				KSBizConfig.uitype = configOrigin.uitype;
				KSBizConfig.navCertTabUse = configOrigin.navCertTabUse;
				KSBizConfig.navCertTabList = configOrigin.navCertTabList;
				KSBizConfig.navCertTabDefault = configOrigin.navCertTabDefault;
				KSBizConfig.navCertButtonCount = configOrigin.navCertButtonCount;
				KSBizConfig.yessignFinCert.signType = configOrigin.yessignFinCert.signType;
				KSBizConfig.yessignFinCert.algorithms = configOrigin.yessignFinCert.algorithms;
				completeCallback(result, context);
			}
			KeySharpBiz.sign(data, callbackfn, options);
		});
    },

    ucpidSign : function(complete, options){
		KeySharpBiz.ucpidDataSign("", complete, options);
    },

    ucpidDataMultiSign : function(data, complete, options){
    	if(!options) options = {};
		options.multiSign = true;
        ...
        options.attributeAsData = true;
        options.signedAttribute = "certStoreType";
        KeySharpBiz.ucpidDataSign(data, complete, options);
    },
	...
	fileHashSign : function(data, filepath, time, complete, options ){
		...
		if (!options) options = {};
		if (!options.hashFileInfo) options.hashFileInfo = {};
		options.hashFileInfo.filepath = filepath;
		options.hashFileInfo.time = time;
		options.addNonce = false;
...
function ksbizGetInitProperty(){
	try{
		...
		properties["selectCertBorder"] = KSBizConfig.selectCertBorder;
...
function ksbiz_loading() {
	...
	try {
		if(!ksbizloadflag){
			TOUCHENEX_CHECK.check([keysharpbizInfo] , "ksbiz_loading_callback");
			ksbizloadflag=true;
		}
	} catch (e) {
		exlog("ksbiz_loading fail", "exception : "+e);
	}
}

function ksbiz_loading_callback(check) {
	exlog("ksbiz_loadingCallback", check);
	try {
        currStatus = check;
        if (currStatus.status) {
            keysharpbizInfo.bizInstalled = currStatus.status;
            TOUCHENEX_LOADING("ksbiz_loadingCallback");
        } else {
        	biz_notInstall(currStatus);
        }
    } catch (e) {
		exlog("ksbiz_loadingCallback", "exception : "+e);
	}
}

function ksbiz_loadingCallback(check) {
//console.log("==============ksbiz_loadingCallback================");	
}

function ksbiz_extensionInstall() {
	if(TOUCHENEX_UTIL.isChrome() || TOUCHENEX_UTIL.isFirefox() || TOUCHENEX_UTIL.isOpera()){
		ksbiz_extensiondownload();
	}
}
