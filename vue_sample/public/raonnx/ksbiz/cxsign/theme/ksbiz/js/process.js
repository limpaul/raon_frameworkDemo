...
var CXCONST = {
	...
	// NAVMENU는 인증서 수단을 선택하는 옆의 메뉴를 뜻한다.
	// PUBLIC : 공동저장소
	// YESSIGN : 금융인증서(개인)
	// YESSIGNCORP : 금융인증서(사업자)
	// YESSIGNINT : 금융인증서(통합)
	"NAVMENU":{
		"PUBLIC" : "publicCert",
		"YESSIGN" : "yessignFinCert",
		"YESSIGNCORP" : "yessignFinCertCorp",
		"YESSIGNINT" : "yessignInt"
	}
};
...
var cxsign = {
	...
	"navCertMenuActionBtn" : {
		"func" : {
			"publicCert" : "initPublic",
			"yessignFinCert" :  "initYessignFinCert",
			"yessignFinCertCorp" : "initYessignFinCert",
			"yessignInt" : "initYessignFinCert"
		}
	},
	"certGrid": [],
	"isbrowser" : false,
...
var init = {
	initConfig : function(request, callback){
		...
		//무설치 스마트 유심 사용시 전자서명 외 나머지에서는 버튼  disable
		if(KSBizConfig.smartUsim != undefined && KSBizConfig.smartUsim.use){
			if(!cxCtrl.isSignPage()){
				for(var key in cxsign.storageDisable){
					if(cxCtrl.getSelectDrive(key).length <= 0){
						cxCtrl.setStorageDisable(key, "USIM");
					} else {
						if(cxsign.storageDisable[key].indexOf("USIM") == -1 ){
							cxCtrl.setStorageDisable(key,"USIM|" + cxCtrl.getStorageDisable(key));
						}
					}
				}
			} else {
				if(KSBizConfig.prepareCertStore.indexOf("|USIM") != -1){
					KSBizConfig.prepareCertStore = KSBizConfig.prepareCertStore.replace("|USIM","");
				}
			}
		}

		// 인증서 수단을 사용할 경우 추가
		if ( KSBizConfig.navCertTabUse != undefined && KSBizConfig.navCertTabUse && (cxCtrl.isSignPage() || cxCtrl.isManageCertPage())){
			ui.initNavCertMenu();
		} 
...
var cxCtrl = {
	...
	getNavMenuConfig : function(value){
		var len = Object.keys(CXCONST.NAVMENU).length;
		for(var name in CXCONST.NAVMENU){
			if(CXCONST.NAVMENU[name] == value){
				return name;
			}
		}
		return "";
	},
	gettNavMenuavActionBtnFunc: function(funcName){
		return cxsign.navCertMenuActionBtn.func[funcName];
	}
};
...
var secureKeyCtrl = {
	...
	initEncPwd : function(callback){
		...
		if(!KSBizConfig.disableScreenKeyboard){
			...
			$(".btn_keyboard").on("keydown", function(e){
				if(e.keyCode == 13) {
				   $(this).children().click();   
				}
			 });
				...
				TOUCHENEX_CHECK.check([touchenexInfo], function(result){
					if(result && result.status){
						...
						if(chkMacVersion){
							...
							if(typeof TK_bInit != "undefined" && TK_bInit != 1){
								TK_Loading();
								if(TK_bInit != 1){
					...
					for(var j=0; j < frm.elements.length; j++){
						...
						if(chkElement.type == "password"){
							...
							if(secureKeyCtrl.kpad == "transkey"){
								...
								chkElement.setAttribute("tabindex", "-1");
