<template>
          <div>
                ...
                <div>
                    <button @click="isInstallCheck">PKI 솔루션 key#biz 설치체크</button>
                </div>
                <div>
                    ...
                    <button @click="certSelfCheck">전자서명 본인확인</button>
                    
                    <button @click="certLogin">전자서명 로그인(nonce 난수 값 재사용방지 지원)</button>

                    <button @click="encryptData">구간암호화 값 생성</button>
                </div>
                <div>
                    <input type="text" id="certVerifyText" :style="{width:'500px'}" placeholder="인증서의 주민번호 또는 사업자 번호에대한 R값이 표시됩니다" readOnly/>
                </div>
                <div>
                    <textarea id="certSignVerifyResultArea" rows="30" cols="120" :style="{overflow: scroll}" placeholder="인증서 전자서명 결과값이 여기 표시됩니다" readOnly></textarea>
                </div>
                <button @click="signAction">서버로 전자서명 값 전송</button>
                <button @click="encryptAction">서버로 구간 암호화 값 전송</button>
            </div>
</template>
...
const isInstallCheck = () =>{
    window.KeySharpBiz.isInstall(false, (result)=>{
            if(result){
                alert("[컨트롤 설치확인] 설치");
            }else{
                alert("[컨트롤 설치확인] 미설치");
...
const certSelfCheck = ()=>{
     window.KSBizConfig.nonceUrl = '/api/ksbiz/nonce.jsp'
     const usernum = document.getElementById('idn').value;
     if(usernum == ''){
         alert('인증서 사업자 번호 또는 주민번호를 입력해주세요')
         return;
     }
     window.KeySharpBiz.sign(document.getElementById('idn').value, (res)=>{
...
const certLogin = ()=>{
     ...
     window.KeySharpBiz.login(plainData.value, (res)=>{
         document.getElementById('certVerifyText').value = res.vidRandom;
         document.getElementById('certSignVerifyResultArea').value = res.data;
     })
}
...
const encryptData = ()=>{
    window.KSBizConfig.lineServerURL = '/api/ksbiz/line.jsp';
    var plainData = document.getElementById('plainText');
    if(plainData.value == ''){
        plainData.value = '가나다라마사';
    }
    window.KeySharpBiz.lineEncrypt({"data" : plainData.value}, (res)=>{
        document.getElementById('certSignVerifyResultArea').value = res.data;
    });
}
...
const encryptAction = ()=>{
    ...
    axios.post('/api/ksbiz/lineEncryptAction.jsp', {
        ...
        if(res.data.status == 0){
            window.KeySharpBiz.lineDecryptAsync(res.data.serverEncData, function(plain){
                if(plain.status == 1){
