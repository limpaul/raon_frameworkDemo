...
import MtranskeyEdit from "./MtranskeyEdit";
import LinkComponent from "../LinkComponent"
import { useNavigate } from "react-router-dom";
function MtranskeySample(){
    ...
    useEffect(()=>{
        ...
        if(!isMobile){
            alert('모바일웹 가상키패드는 모바일 웹 화면에서 사용필요');
            window.location.href='/';  // navtigate를 사용하지 않은 이유는 index.html에서 새로 고침을 함으로서 모바일 웹 스크립트 로드가 되어야 하기 때문이다
        }else{
            window.mtk_useTranskey=true; // input 클릭 이벤트시 가상키패드가 강제로 활성화 되게 하는 옵션 /transkey_mobile/transkey_mobile_config.js에서  설정 가능 
    ...
    return (
        <>
            ...
            <div>
                ...
                <MtranskeyEdit type='password' id='numberkeypad' name='numberkeypad' keypadType='number' placeholder='numberkeypad' keypadBottom={true} isDecodeBtn={true}/>
            </div>

            <h2>initmTranskey() 함수 호출이후 동적으로 input 생성될 경우 tk.setKeyboard함수를 사용합니다</h2>
            <div>
            ...
            <div>
                {
                    dynamicInputs.map((item)=>(
                        <div key={item.id} style={{
                            ...
                            <MtranskeyEdit type={item.type} id={item.id} name={item.name} keypadType={item.keypadType} placeholder={item.placeholder} isDynamic={true} isDecodeBtn={true}/>
                            <button onClick={()=>{
...
export default MtranskeySample;