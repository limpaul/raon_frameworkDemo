...
var cxsign = {
	...
	"navActionBtn" : {
		"func" : {
			...
			"yessignFinCert" :  "initYessign",
			"yessignFinCertCorp" : "initYessign",
			"yessignInt" : "initYessign"
		}
	},
	"certList": [],
	"browserCertInfo" : {
...
var init = {
	initConfig : function(request, callback){
		...
		// 인증서 수단을 사용할 경우 탭 메뉴 추가 - 전자서명, 인증서 관리만 속함
		...
		if(cxCtrl.isSignPage()|| cxCtrl.isUpdateCertPage()){
			...
			if(CXCONST.STORAGE.BROWSER == cxsign.currentStorage){
				...
				loadEngine( null, function() {
					ui.loading.end();
					if (!(KSBizConfig.navCertTabUse != undefined && KSBizConfig.navCertTabUse && cxCtrl.isSignPage())){
						ui.initSign();
					}
					parent.XWC.showIFrame();
					if(KSBizConfig.navCertTabUse != undefined && KSBizConfig.navCertTabUse && KSBizConfig.navCertButtonCount != undefined && KSBizConfig.navCertButtonCount == "2" && cxCtrl.isSignPage()){
						ui.naveMenuDefaultClick();
					}
					init.window();
				}, function(){
					alert(uiCtrl.getLang("not_support_browser"));
			...
			if(CXCONST.STORAGE.BROWSER == cxsign.currentStorage){
				...
				loadEngine( null, function() {
					...
					parent.XWC.showIFrame();
					if(KSBizConfig.navCertTabUse != undefined && KSBizConfig.navCertTabUse && KSBizConfig.navCertButtonCount != undefined && KSBizConfig.navCertButtonCount == "2"){
						ui.naveMenuDefaultClick();
					}
					init.window();
				}, function(){
...
var cxCtrl = {
	...
	gettNavMenuavActionBtnFunc: function(funcName){
		return cxsign.navActionBtn.func[funcName];
	},
